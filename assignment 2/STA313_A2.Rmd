...
## Visualization 1: 
### Counts of Bike Thefts in Toronto Neighborhoods
...
neighborhoods= st_read('Toronto_Neighbourhoods/Toronto_Neighbourhoods.shp')
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
pal_var1 <- colorNumeric("YlGnBu", neighborhoods_wgs84$total_bike_thefts, n = 10)
...
leaflet(data = neighborhoods_wgs84) %>%
  ...
  addPolygons(fillColor = ~pal_var1(total_bike_thefts), color = "#BDBDC3", weight = 1,
              fillOpacity = 0.7, smoothFactor = 0.5,
              label = ~paste0(Neighbourh, ": bike thefts counts: ", total_bike_thefts)) %>%
  addLegend(
    pal = pal_var1,
    values = ~total_bike_thefts,
    title = "Counts of bike thefts in each neighbor",
...
- As for the limitations, the plot displays absolute counts, which can be misleading without transformation. For example, thefts might appear higher during certain hours simply because there are more bikes parked at those times. One possible remedy is to divide the theft counts by the estimated number of parked bikes during each hour
...
hourly_theft_df$OCC_HOUR <- as.integer(hourly_theft_df$OCC_HOUR)

hourly_theft_df <- hourly_theft_df %>%
  mutate(
    BIKE_TYPE = case_when(
...
p <- ggplot(hourly_theft_df, aes(x = OCC_HOUR, y = Count, group = BIKE_TYPE, color = BIKE_TYPE)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(
...
- As for the limitations, the plot aggregates data across all days without considering the different time frames, such as weekdays vs. weekends or summer vs. winter. Theft patterns may vary significantly depending on the day and seasons.
